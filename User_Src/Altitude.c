...
void AltitudeCombineThread(void)
{
  ...
  dt = (tPre>0)?((t-tPre)/1000000.0f):0;
	...
	if(!altOffsetInited)	//wait baro to init its offset
		return;
	...
	{
			corr_baro = 0 - FBM320.Altitude - z_est[0];		// MS5611_Altitude baro alt, is postive above offset level. not in NED. z_est is in NED frame. 
			Baro_ALT_Updated=0;
	}
